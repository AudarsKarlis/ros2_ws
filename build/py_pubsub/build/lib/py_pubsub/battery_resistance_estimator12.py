...
class BatteryResistanceEstimator(Node):
    ...
    def listener_callback(self, msg):
        ...
        errorbar_top_list = [float('nan')] * self.num_cells
        ...
        if self.last_current is not None and len(cell_voltages) == self.num_cells:
            ...
            # Calculate resistance and error bars for both directions of current change
            if abs(delta_i) >= 0.1:
                for i in range(self.num_cells):
                    if self.last_cell_voltages[i] is not None:
                        ...
                        if abs(delta_v) >= 0.005:
                            ...
                            errorbar_top_list[i] = ci_val
                            errorbar_bottom_list[i] = ci_val
                        else:
                            self.resistance_values[i].append(float('nan'))
                            self.resistance_times[i].append(rel_time)
                            self.ci_top_history[i].append(float('nan'))
